...
****************************************
I primi 2 plot della temperatura e della Dew fatti su df non sono utilizzabili
Questi sui valori medi sono più chiari
***************************************
```{r}
pivot.table$Avg.Day.Temperature <- aggregate(Temperature.C.~Date,
                                            df, FUN = mean)$Temperature.C.
pivot.table$Avg.Day.Dew <- aggregate(Dew.point.temperature.C.~Date,
                                            df, FUN = mean)$Dew.point.temperature.C.

plot(pivot.table$Date, pivot.table$Avg.Day.Temperature, xlab = "Date",
     ylab = "Avg Temperature", main = "Daily avg Temperature (°C)", type = "l")
plot(pivot.table$Date, pivot.table$Avg.Day.Dew, xlab = "Date",
     ylab = "Avg Dew point Temperature", main = "Daily avg Dew point Temperature (°C)", type = "l")
```
